...
func TestCreateQuestions(t *testing.T) {
	...
	timeout := time.Now().Local().Add(time.Hour*time.Duration(1) +
	...
	timeout1 := time.Now().Local().Add(time.Hour*time.Duration(1) +
	...
	survey = dao.Survey{SurveyName: "coffelovers",
		...
		Questions:   questions,
		Status:      false,
		Expiry:      timeout1.String(),
	}

	clctn.Insert(survey)

	timeout2 := time.Now().Local().Add(time.Hour*time.Duration(0) +
		time.Minute*time.Duration(0) +
